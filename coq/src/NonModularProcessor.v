...
Module Spec.
    Local Instance processor_modules : instances :=
      #| 
        ...
        SimpleMemory.mkMem imem;
        SimpleMemory.mkMem dmem;

        FifoSpec.mkFifoSpec to_mmio;
        FifoSpec.mkFifoSpec from_mmio
      |#.
      ...
      (rule
          (if (= {read state_machine} `{fetch})
              (begin 
                  ...
                  (set pc_req {read pc})
                  {enq_req imem (# (* is_write *) 0 (* addr *) pc_req (* data*) 0)})
              abort)).
    Arguments do_fetch /.
       ...
       (rule
        (if (= {read state_machine} `{execute})
            (begin 
                ...
                (if (ismemory decoded)
                    (if (store decoded)
                      {enq_req dmem (# 1 addr_dest val2)}
                      {enq_req dmem (# 0 addr_dest 0)})
                    (if (ismmio decoded)
...
Module Impl.
    ...
    Definition do_fetch :=
       (rule (begin 
            ...
            (set ppc (+ pc_req 4))
            {write pc ppc}
            {enq_req imem (# 0 pc_req 0)} 
            {enq f2d (# fepoch pc_req ppc)})).
    Arguments do_fetch /.
    ...
    Definition do_decode :=
      ...
      (rule 
        (begin
            (set (fepoch pc_received ppc) {first f2d})
            (set data_resp {resp imem})
            (set decoded (decode data_resp))
            (if (has_destination decoded)
            ...
            (set src1 {read1 rf (src1 decoded)})
            (set src2 {read2 rf (src2 decoded)})
            {enq d2e (# src1 src2 pc_received ppc decoded fepoch)})).
    Arguments do_decode /.

    Definition do_execute :=
      (rule 
        (begin
            (set (val1 val2 pc coming_pc decoded fepoch) {first d2e})
            {deq d2e}
            (set cur_pc coming_pc)
            (set cur_epoch {read epoch})
            (if (= fepoch cur_epoch)
              (begin 
                (if {read processing_st_mmio} abort pass)
                (set output_alu (alu (# decoded val1 val2))) 
                (set ppc (nextpc (# cur_pc val1)))
                (if (not (= ppc coming_pc)) 
                (* Handle misprediction *)
                    (begin 
                        {write pc ppc}
                        {write epoch (+ 1 cur_epoch)}
                        ) 
                    pass)
                (if (ismemory decoded)
                  (begin 
